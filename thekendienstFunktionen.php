...
function aufklappenListederZeitfenster($ID, $AufstellungsName, $editierbar=true) { //Ruft eine Liste aller Zeitfenster zur Mutter-Veranstaltung auf ($ID)
	...
	$i=0;
	(int) $further_i=null;
	$sql='SELECT IDZeitfenster, Tag, KommentarZeitfenster, Startzeit, Endzeit, AnzahlMitarbeiter, IDMitarbeiter FROM '.$table_prefix.'thekendienst WHERE (AufstellungsID="'.$ID.'" AND Archiv!="1") ORDER BY AufstellungsID, IDZeitfenster, Tag, Startzeit, IDMitarbeiter' ; //Abfrage der Einträge zur aktuellen Veranstaltung ($ID)
	$sql2='SELECT IDZeitfenster, Tag, KommentarZeitfenster, Startzeit, Endzeit, AnzahlMitarbeiter, IDMitarbeiter FROM '.$table_prefix.'thekendienst WHERE (AufstellungsID="'.$ID.'") ORDER BY AufstellungsID, IDZeitfenster, Tag, Startzeit, IDMitarbeiter' ; //Abfrage der Einträge zur aktuellen Veranstaltung ($ID)
...
function AufstellungermittelnAdmin() {//
	...
	global $table_prefix;
	(string) $rueckgabe="";
	$tabelle=$wpdb->get_results('SELECT AufstellungsID, AufstellungsName FROM '.$table_prefix.'thekendienst WHERE Archiv!="1" GROUP BY AufstellungsID ORDER BY AufstellungsID', ARRAY_A); //Holt alle vorhanden Einträge aus der Datenbank
	if(is_array($tabelle)) {
		$letzterEintrag=end($tabelle);
		/*if($letzterEintrag["AufstellungsID"]!=count($tabelle)) {//überprüft (rudimentär) die Validität der Tabelle. Beide Arrays gleiche Anzahl von Elementen? Wenn nicht wird nachfolgende Fehlermeldung ausgegeben...
